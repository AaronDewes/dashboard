  ...
  methods: {
    ...
    openApp(event) {
      if (this.torOnly && window.location.origin.indexOf(".onion") < 0) {
        ...
        alert(
          `${this.name} can only be used over Tor. Please access your node in a Tor browser on your remote access URL (Settings > Tor > Remote Access URL) to open this app.`
        );
        return;
      }
