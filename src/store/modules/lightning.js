...
const actions = {
  async getStatus({ commit }) {
    const status = await API.get(
      `${import.meta.env.VITE_MIDDLEWARE_API_URL}/v1/lnd/info/status`
    );
    if (status) {
    ...
    // if (!status.unlocked) {
    //   await sleep(30000);
    //   const { unlocked } = await API.get(`${import.meta.env.VITE_MIDDLEWARE_API_URL}/v1/lnd/info/status`);
  ...
  async getSync({ commit }) {
    const sync = await API.get(
      `${import.meta.env.VITE_MIDDLEWARE_API_URL}/v1/lnd/info/sync`
    );
  ...
  async getLndPageData({ commit, dispatch }) {
    const data = await API.get(
      `${import.meta.env.VITE_MIDDLEWARE_API_URL}/v1/pages/lnd`
    );
  ...
  async getConnectionCode({ commit }) {
    const uris = await API.get(
      `${import.meta.env.VITE_MIDDLEWARE_API_URL}/v1/lnd/info/uris`
    );
  ...
  async getBalance({ commit }) {
    const balance = await API.get(
      `${import.meta.env.VITE_MIDDLEWARE_API_URL}/v1/lnd/wallet/lightning`
    );
  ...
  async getChannels({ commit }, preFetchedChannels = []) {
    ...
    if (preFetchedChannels.length) {
      ...
      rawChannels = await API.get(
        `${import.meta.env.VITE_MIDDLEWARE_API_URL}/v1/lnd/channel`
      );
    }
  ...
  async getTransactions({ commit, state }) {
    ...
    const invoices = await API.get(
      `${import.meta.env.VITE_MIDDLEWARE_API_URL}/v1/lnd/lightning/invoices`
    );
    const payments = await API.get(
      `${import.meta.env.VITE_MIDDLEWARE_API_URL}/v1/lnd/lightning/payments`
    );
    ...
    for (let tx of newOutgoingTransactions) {
      ...
      try {
        const invoiceDetails = await API.get(
          ...
          }/v1/lnd/lightning/invoice?paymentRequest=${tx.paymentRequest}`
        );
  ...
  async getLndConnectUrls({ commit }) {
    const urls = await API.get(
      `${import.meta.env.VITE_MANAGER_API_URL}/v1/system/lndconnect-urls`
    );
    if (urls) {
