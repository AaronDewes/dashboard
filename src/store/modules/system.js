...
const actions = {
  async getVersion({ commit }) {
    const data = await API.get(
      `${import.meta.env.VITE_MANAGER_API_URL}/v1/system/info`
    );
  ...
  async getApi({ commit }) {
    const api = await API.get(
      `${import.meta.env.VITE_MIDDLEWARE_API_URL}/ping`
    );
    commit("setApi", {
  ...
  async getManagerApi({ commit }) {
    const api = await API.get(`${import.meta.env.VITE_MANAGER_API_URL}/ping`);
  ...
  async getAvailableUpdate({ commit }) {
    const update = await API.get(
      `${import.meta.env.VITE_MANAGER_API_URL}/v1/system/get-update`
    );
  ...
  async getUpdateStatus({ commit }) {
    const status = await API.get(
      `${import.meta.env.VITE_MANAGER_API_URL}/v1/system/update-status`
    );
  ...
  async getBackupStatus({ commit }) {
    const status = await API.get(
      `${import.meta.env.VITE_MANAGER_API_URL}/v1/system/backup-status`
    );
  ...
  async getDebugResult({ commit }) {
    const result = await API.get(
      `${import.meta.env.VITE_MANAGER_API_URL}/v1/system/debug-result`
    );
  ...
  async debug({ commit }) {
    const result = await API.post(
      `${import.meta.env.VITE_MANAGER_API_URL}/v1/system/debug`
    );
  ...
  async shutdown({ commit }) {
    ...
    // Shutting down
    const result = await API.post(
      `${import.meta.env.VITE_MANAGER_API_URL}/v1/system/shutdown`
    );
    if (!result) {
    ...
    const pollIfDown = window.setInterval(async () => {
      const { version } = await API.get(
        `${import.meta.env.VITE_MANAGER_API_URL}/ping`
      );
      if (!version) {
  ...
  async reboot({ commit }) {
    ...
    // Rebooting
    const result = await API.post(
      `${import.meta.env.VITE_MANAGER_API_URL}/v1/system/reboot`
    );
    if (!result) {
  ...
  async getStorage({ commit }) {
    const storage = await API.get(
      `${import.meta.env.VITE_MANAGER_API_URL}/v1/system/storage`
    );
  ...
  async getRam({ commit }) {
    const ram = await API.get(
      `${import.meta.env.VITE_MANAGER_API_URL}/v1/system/memory`
    );
    if (ram && ram.total) {
  ...
  async getIsUmbrelOS({ commit }) {
    const isUmbrelOS = await API.get(
      `${import.meta.env.VITE_MANAGER_API_URL}/v1/system/is-umbrel-os`
    );
    commit("setIsUmbrelOS", !!isUmbrelOS);
  },
  async getCpuTemperature({ commit }) {
    const cpuTemperature = await API.get(
      `${import.meta.env.VITE_MANAGER_API_URL}/v1/system/temperature`
    );
    if (cpuTemperature) {
  ...
  async getUptime({ commit }) {
    const uptime = await API.get(
      `${import.meta.env.VITE_MANAGER_API_URL}/v1/system/uptime`
    );
    if (uptime) {
